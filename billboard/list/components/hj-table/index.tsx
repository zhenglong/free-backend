...
export interface HjTableField {
    title?: string;
    key?: string;
    type?: TableField;
}

export interface HjTableProps extends TableProps<TableDataItem> {
    fields: (string | HjTableField)[];
    actions?: ActionButtonType[];
    onDataSourceTransform?: (dataSource: any) => any;
}

interface HjTableState {
    dataSource: any[]
}

export default class HjTable extends React.Component<HjTableProps, HjTableState> {
    fieldsMeta: ColumnProps<TableDataItem>[] = [];

    constructor(props: HjTableProps) {
        super(props);
        // 需要把自定义参数转换成antd的参数
        for (let field of props.fields) {
            ...
            if (isFieldString) {
                ...
                let tempField = field as HjTableField;
                column = {
