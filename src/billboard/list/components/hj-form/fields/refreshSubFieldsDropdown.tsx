...
export interface HjRefreshPageDropdownProps extends SelectProps, HjFieldProps {
 ...
 * 
 */
export default class HjRefreshSubFieldsDropdownField extends Component<HjRefreshPageDropdownProps> implements HjFieldWidget {
    innerWidget: RefObject<Select>;
    value: any;
    constructor(props: HjRefreshPageDropdownProps) {
        super(props);
    ...
    componentDidMount() {
        ...
        //     remoteData.fields(this.props.hjForm.RefeshFieldsDataView).then((fields: FormFieldDef[]) => {
        //         // 恢复非dynamic字段的值
        ...
        //             }
        //         });
        //         this.props.hjForm.updateFields(fields);
        //     });
        // }
    }
    ...
    onChange(val, opt) {
        ...
        global.actions.updateQueryCondition(this.getValue());
        // 保存非dynamic字段的值
        let savedValue = this.props.hjForm.getDataView(true);
        // 知道哪个字段的值变化了，然后把RefreshFieldsDataView中index大于它的值去掉
        let changedIndex = this.props.index;
        let dataView = this.props.hjForm.RefeshFieldsDataView;
            ...
            <fieldCollectorContext.Consumer>
                {
                    (ctx: FieldCollector) => {
                        ...
                        return (
                            <Form.Item label={this.props.title} className="hj-refresh-subfields-field">
